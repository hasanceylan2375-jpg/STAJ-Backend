...
namespace STAJ.Controllers
{
    ...
    public class MusteriController : ControllerBase
    {
        ...
        public async Task<IActionResult> FotografYukle(IFormFile file)
        {
            ...
            var uzantilar = new[] { ".jpg", ".jpeg", ".png", ".webp" }; var uzanti = Path.GetExtension(file.FileName).ToLowerInvariant();
            if (!uzantilar.Contains(uzanti)) return BadRequest("Sadece JPG, JPEG, PNG veya WEBP dosyaları yüklenebilir.");
            var uploadsPath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "uploads"); Directory.CreateDirectory(uploadsPath);
            await using var yuklenenDosyaStream = file.OpenReadStream(); var yuklenenHash = Convert.ToHexString(await SHA256.HashDataAsync(yuklenenDosyaStream));
            foreach (var mevcutDosya in Directory.GetFiles(uploadsPath)) { await using var mevcutDosyaStream = System.IO.File.OpenRead(mevcutDosya); var mevcutHash = Convert.ToHexString(await SHA256.HashDataAsync(mevcutDosyaStream)); if (yuklenenHash == mevcutHash) return BadRequest("Bu fotoğraf daha önce yüklenmiş."); }
        ...
        [HttpGet]
        [EnableRateLimiting("read")]
        public IActionResult Getir([FromQuery] string? search = null, [FromQuery] string? sort = null, [FromQuery] int page = 1, [FromQuery] int pageSize = 5) { var musteriler = _service.Getir(search, sort, page, pageSize); return Ok(new DataResult<object>(true, _localizer["CustomersRetrieved"], musteriler)); }

        [HttpGet("excel")]
        [Authorize(Policy = "AdminOnly")]
        [EnableRateLimiting("read")]
        public IActionResult ExcelAktar([FromQuery] string? search = null, [FromQuery] string? sort = null)
        { var musteriler = _service.Getir(search, sort, 1, 10000); using var workbook = new XLWorkbook(); var worksheet = workbook.Worksheets.Add("Müşteriler"); worksheet.Cell(1, 1).Value = "ID"; worksheet.Cell(1, 2).Value = "Ad"; worksheet.Cell(1, 3).Value = "Soyad"; worksheet.Cell(1, 4).Value = "Telefon"; worksheet.Cell(1, 5).Value = "E-posta"; for (int i = 0; i < musteriler.Count; i++) { var m = musteriler[i]; var row = i + 2; worksheet.Cell(row, 1).Value = m.Id; worksheet.Cell(row, 2).Value = m.Ad; worksheet.Cell(row, 3).Value = m.Soyad; worksheet.Cell(row, 4).Value = m.Telefon; worksheet.Cell(row, 5).Value = m.Email; } worksheet.Range(1, 1, 1, 5).Style.Font.Bold = true; worksheet.Columns().AdjustToContents(); using var stream = new MemoryStream(); workbook.SaveAs(stream); return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "musteriler.xlsx"); }
        ...
        [HttpGet("{id}")]
        [EnableRateLimiting("read")]
        public IActionResult IdyeGoreGetir(int id) { var musteri = _service.IdyeGoreGetir(id); if (musteri == null) return NotFound(new DataResult<object>(false, _localizer["CustomerNotFound"])); return Ok(new DataResult<Musteri>(true, _localizer["CustomerRetrieved"], musteri)); }
