...
namespace STAJ.Controllers
{
    ...
    public class MusteriController : ControllerBase
    {
        ...
        public MusteriController(IMusteriService service, IStringLocalizer<SharedResource> localizer, IValidator<Musteri> validator, AppDbContext context, IDomainEventDispatcher eventDispatcher)
        {
            _service = service;
        ...
        public async Task<IActionResult> FotografYukle(IFormFile file)
        {
            ...
            if (!IzinliUzantilar.Contains(uzanti)) return BadRequest("Sadece JPG, JPEG, PNG veya WEBP dosyaları yüklenebilir.");
            ...
            foreach (var mevcutDosya in Directory.GetFiles(uploadsPath))
            {
                ...
                if (yuklenenHash == Convert.ToHexString(await SHA256.HashDataAsync(mevcutDosyaStream)))
        ...
        public async Task<IActionResult> Ekle(Musteri musteri, [FromHeader(Name = "Idempotency-Key")] string? idempotencyKey)
        {
            ...
            {
                if (existingRecord.ExpireAt <= DateTime.UtcNow) _context.IdempotencyRecords.Remove(existingRecord);
                else
                {
            ...
            if (!validationResult.IsValid) return ValidationError(validationResult.Errors.Select(x => x.ErrorMessage));
            ...
            try
            {
                ...
                _context.IdempotencyRecords.Add(new IdempotencyRecord { Key = idempotencyKey, RequestHash = requestHash, Response = JsonSerializer.Serialize(response), StatusCode = StatusCodes.Status200OK, ExpireAt = DateTime.UtcNow.AddHours(24) });
